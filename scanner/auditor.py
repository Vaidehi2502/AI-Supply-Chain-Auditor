...
def audit_model(model_path: str, requirements_path: str = None) -> dict:
    ...
    print(f"[1/3] Running serialization scan...", file=sys.stderr)
    report["scan_results"]["serialization"] = scan_serialization(model_path)

    # Run weight scan
    print(f"[2/3] Running weight anomaly scan...", file=sys.stderr)
    report["scan_results"]["weights"] = scan_weights(model_path)

    # Run dependency scan if requirements provided
    if requirements_path:
        print(f"[3/3] Running dependency scan...", file=sys.stderr)
        report["scan_results"]["dependencies"] = scan_requirements(requirements_path)
    else:
        print(f"[3/3] No requirements.txt provided, skipping dependency scan.", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    sys.exit(1 if result["overall_risk"] in ("DANGER", "ERROR") else 0)